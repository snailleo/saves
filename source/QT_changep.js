$('document').ready(function() {
	if (getCookie('qtpopup') == '') {
		// setTimeout(function(){
		// setCookie('qtpopup', '1');
		// _hmt.push(['_trackEvent', 'wx', 'wx_show_popup']);
		// $('.popup').toggle();
		// }, 20000);
	}
	...
	addEvent('timeupdate', qtplayer, function() {
	...
	addEvent('durationchange', qtplayer, function() {
	...
	addEvent('waiting', qtplayer, function() {
		console.log('waiting');
		writeToLogger('waiting');
		playing = false;
		changeStat('load');
	});
	addEvent('pause', qtplayer, function() {
		// on stop
		console.log('pause');
		writeToLogger('pause');
		playing = false;
	...
	addEvent('playing', qtplayer, function() {
		// on playing
		console.log('playing');
	...
	addEvent('play', qtplayer, function() {
		// on playing
		console.log('play');
		writeToLogger('play');
		playing = true;
	...
	addEvent('ended', qtplayer, function() {
		// on finish
		console.log('ended');
		writeToLogger('ended');
		playing = false;
	...
	addEvent('error', qtplayer, function() {
		// on error
		console.log('error');
		writeToLogger('error');
		playing = false;
	...
	$('#quick-list').click(function() {
		...
		if ($('.page-live').css('display') == 'none') {
			...
			// $('.cover-bg-blur').css('opacity', '0.9');
		} else {
			$('#quick-list').find('h5').text('查看节目单');
			// $('.cover-bg-blur').css('opacity', '0.6');
		}
	});
